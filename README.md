...
Geese Gaze 监控系统是一个专为实验室环境设计的自动化扫码处理和数据传递系统。它能够自动监控指定文件夹中的图像，识别图片试管板中的二维码（QR码）和数据矩阵码（DM码），并将处理结果发送到指定的服务器接口。系统采用多模式识别技术，能够高效处理不完整或模糊的二维码，支持QR码和DM码识别模式切换，大大提高了实验室数据采集的准确性和效率。
